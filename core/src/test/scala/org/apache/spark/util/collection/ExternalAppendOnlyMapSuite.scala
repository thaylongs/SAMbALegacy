...
import br.uff.spark.{DataElement, DataflowProvenance, DataflowUtils, Task}
...
class ExternalAppendOnlyMapSuite extends SparkFunSuite with LocalSparkContext {
  ...
  testTask.description = "TestTaskExternalAppendOnlyMapSuite"
  ...
  private def createExternalMap[T] = {
    ...
    new ExternalAppendOnlyMap[T, T, ArrayBuffer[T]](testTask,
  ...
  test("single insert") {
    ...
    map.insert(1, 10, null)
    val it = map.iterator
    assert(it.hasNext)
    val kv = it.next()
    assert(kv.value._1 === 1 && kv.value._2 === ArrayBuffer[Int](10))
    assert(!it.hasNext)
    sc.stop()
  }
  ...
  test("multiple insert") {
    ...
    val map = createExternalMap[Int]
    map.insert(1, 10, null)
    map.insert(2, 20, null)
    map.insert(3, 30, null)
    val it = map.iterator
    assert(it.hasNext)
    assert(DataflowUtils.extractFromIterator(it).toSet === Set[(Int, ArrayBuffer[Int])](
  ...
  test("insert with collision") {
    ...
    map.insertAll(Seq(
      ...
      (1, 100),
      (2, 200),
      (1, 1000)).map(a=>DataElement.of(a)))
    val it = map.iterator
    assert(it.hasNext)
    val result = it.toSet[DataElement[(Int, ArrayBuffer[Int])]].map(kv => (kv.value._1, kv.value._2.toSet))
  ...
  test("ordering") {
    ...
    map1.insert(1, 10, null)
    map1.insert(2, 20, null)
    map1.insert(3, 30, null)

    val map2 = createExternalMap[Int]
    map2.insert(2, 20, null)
    map2.insert(3, 30, null)
    map2.insert(1, 10, null)

    val map3 = createExternalMap[Int]
    map3.insert(3, 30, null)
    map3.insert(1, 10, null)
    map3.insert(2, 20, null)
    ...
    var kv2 = it2.next()
    var kv3 = it3.next()
    assert(kv1.value._1 === kv2.value._1 && kv2.value._1 === kv3.value._1)
    assert(kv1.value._2 === kv2.value._2 && kv2.value._2 === kv3.value._2)
  ...
  test("null keys and values") {
    ...
    map.insert(2, 6, null)
    map.insert(3, 7, null)
    map.insert(4, nullInt, null)
    map.insert(nullInt, 8, null)
    map.insert(nullInt, nullInt, null)
    val result = map.iterator.toSet[DataElement[(Int, ArrayBuffer[Int])]].map(kv => (kv.value._1, kv.value._2.sorted))
  ...
  test("ExternalAppendOnlyMap shouldn't fail when forced to spill before calling its iterator") {
    ...
    map.insertAll((1 to size).iterator.map(_.toString).map(i => DataElement.of((i, i))))
  ...
  test("spilling with hash collisions") {
    ...
    map.insertAll((1 to size).iterator.map(_.toString).map(i => DataElement.of((i, i))))
    collisionPairs.foreach { case (w1, w2) =>
      map.insert(w1, w2, null)
      map.insert(w2, w1, null)
    }
  ...
  test("spilling with many hash collisions") {
    ...
    val context = MemoryTestingUtils.fakeTaskContext(sc.env)
    val map =
      new ExternalAppendOnlyMap[FixedHashObject, Int, Int](testTask, _ => 1, _ + _, _ + _, context = context)

    // Insert 10 copies each of lots of objects whose hash codes are either 0 or 1. This causes
    // problems if the map fails to group together the objects with the same code (SPARK-2043).
    for (i <- 1 to 10) {
      for (j <- 1 to size) {
        map.insert(FixedHashObject(j, j % 2), 1, null)
      }
    }
    assert(map.numSpills > 0, "map did not spill")
  ...
  test("spilling with hash collisions using the Int.MaxValue key") {
    ...
    (1 to size).foreach { i => map.insert(i, i, null) }
    map.insert(Int.MaxValue, Int.MaxValue, null)
    assert(map.numSpills > 0, "map did not spill")
  ...
  test("spilling with null keys and values") {
    ...
    map.insertAll((1 to size).iterator.map(i => DataElement.of((i, i))))
    map.insert(null.asInstanceOf[Int], 1, null)
    map.insert(1, null.asInstanceOf[Int], null)
    map.insert(null.asInstanceOf[Int], null.asInstanceOf[Int], null)
    assert(map.numSpills > 0, "map did not spill")
