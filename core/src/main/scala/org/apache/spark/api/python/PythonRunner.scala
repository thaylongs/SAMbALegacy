...
private[spark] abstract class BasePythonRunner[IN, OUT](
  ...
  def compute(
      inputIterator: Iterator[DataElement[IN]],
      partitionIndex: Int,
      context: TaskContext): Iterator[DataElement[OUT]] = {
  ...
  abstract class ReaderIterator(
      ...
      released: AtomicBoolean,
      context: TaskContext)
    extends Iterator[DataElement[OUT]] {

    private var nextObj: DataElement[OUT] = _
    ...
    override def next(): DataElement[OUT] = {
      if (hasNext) {
        val obj = nextObj
        nextObj = null.asInstanceOf[DataElement[OUT]]
        obj
      } else {
        Iterator.empty.next()
      }
     ...
     */
    protected def read(): DataElement[OUT]
    ...
    protected val handleException: PartialFunction[Throwable, DataElement[OUT]] = {
      ...
      case e: Exception if env.isStopped =>
        ...
        null.asInstanceOf[DataElement[OUT]]  // exit silently
...
private[spark] class PythonRunner(
  ...
  protected override def newWriterThread(
      env: SparkEnv,
      worker: Socket,
      inputIterator: Iterator[DataElement[Array[Byte]]],
      partitionIndex: Int,
      context: TaskContext): WriterThread = {
  ...
  protected override def newReaderIterator(
      ...
      worker: Socket,
      released: AtomicBoolean,
      context: TaskContext): Iterator[DataElement[Array[Byte]]] = {
    new ReaderIterator(stream, writerThread, startTime, env, worker, released, context) {

      protected override def read(): DataElement[Array[Byte]] = {
        ...
        try {
          stream.readInt() match {
            case length if length > 0 =>
              ...
              stream.readFully(obj)
              DataElement.of(obj)
            case 0 => DataElement.of(Array.empty[Byte])
